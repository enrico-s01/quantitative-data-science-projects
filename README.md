# quantitative-data-science-projects
# Quantitative Modelling and Data Science Portfolio

Selected academic projects developed during my M.Sc. in Stochastic and
Data Science at the University of Turin and my Erasmus exchange at
Institut Polytechnique de Paris.

## Projects

### ATP Finals Winner Prediction
Weighted Elo model and Monte Carlo simulation of the complete ATP Finals
tournament, including historical backtesting from 2000 to 2024.

**Methods:** Elo ratings, Monte Carlo simulation, probabilistic modelling,
backtesting, pandas, NumPy.

### FOMC Statements and Bitcoin Regime Switching
Analysis of whether the hawkish or dovish tone of FOMC statements helps
explain changes in Bitcoin volatility regimes.

**Methods:** NLP, financial time series, regime-switching models,
feature engineering, statistical analysis.

### Asian Option Pricing
Derivation and numerical solution of the pricing PDE for arithmetic Asian
options, including stability and convergence analysis.

**Methods:** stochastic calculus, PDEs, finite-difference methods,
option pricing, numerical analysis.

### ResNet vs Neural ODE
Implementation and comparison of ResNet and Neural ODE architectures for
human activity recognition, including robustness analysis under input noise.

**Methods:** PyTorch, deep learning, neural ODEs, classification,
robustness analysis.

### Breast Cancer Classification
Comparison of classical classification algorithms on the Wisconsin Breast
Cancer dataset.

**Methods:** KNN, decision trees, Naive Bayes, SVM, RIPPER,
cross-validation and model evaluation.

## Main Technologies

Python, NumPy, pandas, scikit-learn, PyTorch, Matplotlib.
